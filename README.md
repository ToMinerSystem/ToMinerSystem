...
### 👉 **Linux 安装**
   ...
   <p>&emsp;&emsp;Ubuntu x86_64 服务器运行以下 Shell 指令安装 ToMinerSystem：</p>

   ```sh
   bash <(curl -fsSL https://github.com/EvilGenius-dot/shortcut/raw/main/Readme/92/install.sh) install
   ```

   <p>&emsp;&emsp;安装脚本会下载程序、随机生成 Web 端口和初始密码，并注册 systemd 开机自启动服务。</p>
...
### 👉 **Windows 安装**
   ...
   ```text
   windows/ToMinerSystem-Server.exe
   ```

   <p>&emsp;&emsp;下载地址：<a href="windows/ToMinerSystem-Server.exe">ToMinerSystem-Server.exe</a></p>
...
```text
TMS/windows/ToMinerSystem-TMS.exe
```

<p>&emsp;&emsp;下载地址：<a href="TMS/windows/ToMinerSystem-TMS.exe">ToMinerSystem-TMS.exe</a></p>

<p>&emsp;&emsp;Ubuntu x86_64 版本运行以下指令安装：</p>

```sh
bash <(curl -fsSL https://github.com/EvilGenius-dot/shortcut/raw/main/Readme/92/TMS/install.sh) install
```
...
<p>&emsp;&emsp;Ubuntu TMS 常用管理指令：</p>
...
README.md
install.sh
linux/tominersystem-server-linux-x86_64
windows/ToMinerSystem-Server.exe
TMS/install.sh
TMS/linux/tms-local-linux-x86_64
TMS/windows/ToMinerSystem-TMS.exe
```

<p>&emsp;&emsp;Ubuntu 安装脚本会校验下载程序的 SHA-256。运行 Windows 程序前，建议同时核对发布页提供的文件校验值。</p>
...
<p>&emsp;&emsp;Ubuntu 安装脚本开头集中设置了 <code>VERSION</code>、<code>APP_NAME</code>、<code>APP_ID</code>、<code>DOWNLOAD_HOST</code> 和 <code>SERVICE_NAME</code>，自行托管发行文件时可以在该区域修改下载地址和品牌信息。</p>
...
ToMinerSystem v5.0.4 · Windows / Ubuntu · 默认服务费 0%
