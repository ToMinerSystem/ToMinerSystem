...
readonly EXPECTED_SHA256="6e0e24a464d0a98ea8edd3f11c3dda21226a68b7eb8123708aada9f6514465f5"
...
readonly RELEASE_API_URL="https://api.github.com/repos/ToMinerSystem/ToMinerSystem/contents/linux?ref=main"
...
latest_published_version() {
  ...
  latest="$(printf '%s\n' "${response}" \
    | tr '{' '\n' \
    | sed -nE 's/.*"name"[[:space:]]*:[[:space:]]*"ToMinerSystem-([0-9]+\.[0-9]+\.[0-9]+)".*/\1/p' \
    | sort -V \
    | tail -n 1)"
  [[ "${latest}" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] \
    || fail "GitHub linux 目录中没有可安装的正式版本程序"
  echo "${latest}"
}
