...
# ToMinerSystem Ubuntu x86_64 二进制安装器。
...
readonly EXPECTED_SHA256="392e1b844af11f80a425b07e8e47ca5943f9ae1a7d90e563d4936f3a34a45c03"
...
require_root_ubuntu_x86_64() {
  ...
  # /etc/os-release 也定义 VERSION。不能 source 到当前 Shell，否则会与上方
  # 只读的应用 VERSION 冲突。这里只读取实际需要的两个字段。
  ...
  [[ "${os_id}" == "ubuntu" ]] || fail "仅支持 Ubuntu，当前系统为 ${os_pretty_name:-unknown}"
...
reset_password() {
  require_root_ubuntu_x86_64 reset-password
  local password
...
change_web_port() {
  ...
  require_root_ubuntu_x86_64 change-web-port
...
run_service_action() {
  local action="$1"
  require_root_ubuntu_x86_64 "${action}"
...
show_menu() {
  ...
  while true; do
    ...
    echo "  0. 退出"
    echo "========================================"
    read -r -p "请选择 [0-8]：" choice || return 0
    case "${choice}" in
      1) install_server ;;
      2) run_service_action start ;;
      3) run_service_action stop ;;
      4) run_service_action restart ;;
      5) show_web_port ;;
      6) change_web_port ;;
      7) uninstall_server ;;
      8) reset_password ;;
...
case "${command_name}" in
  ...
  *)
    echo "用法：sudo bash install.sh [install|start|stop|restart|status|web-port|change-web-port|reset-password|uninstall]" >&2
